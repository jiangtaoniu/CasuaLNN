"""
[**ATTENTION**] This file appears to be a backup and is identical in
functionality to `layers/CasuaLNN_layers.py`. For any future development,
it is recommended to modify `layers/CasuaLNN_layers.py` to avoid
...
def get_num_patches(seq_len: int, patch_len: int, stride: int) -> int:
    """
    ...
    A one-sided padding equal to the stride is implicitly assumed at the end of the sequence.
    ...
    if seq_len <= 0:
        return 0
    # Formula: floor((L + padding - kernel_size) / stride + 1)
    padding = stride
...
class MultiScaleSeasonMixing(nn.Module):
    ...
    coarser-resolution scales by learning a projection from the patch space
    of a fine scale to that of a coarse scale.
    """
    ...
    def __init__(self, configs):
        ...
        # Pre-calculate the number of patches and define projection layers for each scale
        for i in range(configs.down_sampling_layers + 1):
            ...
            # Define the down-sampling projection layers from fine scale `i` to coarse scale `i+1`
            if i < configs.down_sampling_layers:
                ...
                self.down_sampling_layers.append(
                    nn.Sequential(
                        nn.Linear(num_patches_fine, num_patches_coarse),
                        nn.GELU(),
                        nn.Linear(num_patches_coarse, num_patches_coarse),
                    )
                )
    ...
    def forward(self, season_list: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        # Iterate from the finest scale to the coarsest
        for i in range(len(season_list) - 1):
            low_res_season = season_list[i + 1]
            high_res_season = high_res_season.permute(0, 2, 3, 1)  # B, N, D, T (num_patches)
            B, N, D, T_high = high_res_season.shape

            # Project fine-scale representation to coarse-scale patch dimension
            ...
            # Additive fusion: add projected fine-scale info to the coarse-scale representation
            low_res_season = low_res_season.permute(0, 2, 3, 1) + res
            high_res_season = low_res_season.permute(0, 3, 1, 2)  # Update for next iteration's input
...
class MultiScaleTrendMixing(nn.Module):
    ...
    finer-resolution scales by learning a projection from the patch space
    of a coarse scale to that of a fine scale.
    """
    ...
    def __init__(self, configs):
        ...
        # Define the up-sampling projection layers from coarse scale `i+1` to fine scale `i`
        for i in reversed(range(configs.down_sampling_layers)):
            ...
            if num_patches_coarse <= 0 or num_patches_fine <= 0:
                raise ValueError(f"Calculated zero or negative patches for upsampling from scale {i+1} to {i}.")

            self.up_sampling_layers.append(
                nn.Sequential(
                    nn.Linear(num_patches_coarse, num_patches_fine),
                    nn.GELU(),
                    nn.Linear(num_patches_fine, num_patches_fine),
                )
            )
    ...
    def forward(self, trend_list: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        # Reverse the list to iterate from coarsest to finest scale
        ...
        for i in range(len(trend_list_reverse) - 1):
            ...
            # Project from coarse scale to fine scale dimension
            ...
            # Additive fusion: add projected coarse-scale info to the fine-scale representation
            high_res_trend = high_res_trend.permute(0, 2, 3, 1) + res
            current_low_res_trend = high_res_trend.permute(0, 3, 1, 2) # Update for next iteration's input
            out_trend_list_rev.append(current_low_res_trend)

        out_trend_list_rev.reverse() # Reverse the list back to original order (finest to coarsest)
...
class PDM_Deep_Block(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Select decomposition method based on config
    ...
    def forward(self, feature_list: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        # 1. Decompose features at each scale into seasonal and trend parts
        for i, feature in enumerate(feature_list):
            B, T, N, D = feature.shape
            # Handle edge case where sequence length is too short for decomposition
            if T <= 1:
                season, trend = torch.zeros_like(feature), feature
            else:
        ...
        # 2. Mix seasonal (bottom-up) and trend (top-down) components across scales
        mixed_season_list = self.mixing_multi_scale_season(season_list)
        mixed_trend_list = self.mixing_multi_scale_trend(trend_list)

        # 3. Fuse mixed components and apply residual connection
        out_list = []
