...
def main():
    ...
    parser.add_argument('--model_id', type=str, default='PEMS08_96_12', help='实验的唯一标识符')
    ...
    parser.add_argument('--data', type=str, default='PEMS', help='数据集类型')
    parser.add_argument('--root_path', type=str, default='./dataset/PEMS/', help='数据文件根目录')
    parser.add_argument('--data_path', type=str, default='PEMS08_714.npz', help='数据文件名')
    ...
    parser.add_argument('--label_len', type=int, default=48, help='解码器预填充的起始token长度 (常用于Autoformer等)')
    parser.add_argument('--pred_len', type=int, default=12, help='预测序列长度 (prediction horizon)')
    ...
    # 4. 模型架构通用设置
    parser.add_argument('--d_model', type=int, default=512, help='模型隐藏维度')
    parser.add_argument('--d_ff', type=int, default=2048, help='前馈网络中间层维度')
    parser.add_argument('--enc_in', type=int, default=170, help='编码器输入尺寸 (特征/变量数)')
    parser.add_argument('--dec_in', type=int, default=170, help='解码器输入尺寸 (兼容部分旧模型)')
    parser.add_argument('--c_out', type=int, default=170, help='输出尺寸')
    ...
    parser.add_argument('--distil', action='store_false', help='是否在编码器中使用蒸馏操作(适用于Informer/Autoformer)',
                        default=True)
    parser.add_argument('--decomp_method', type=str, default='dft_decomp', help='序列分解方法, 可选: moving_avg, dft_decomp')
    parser.add_argument('--moving_avg', type=int, default=25, help='移动平均窗口大小')
    parser.add_argument('--down_sampling_layers', type=int, default=2, help='降采样层数')
    ...
    parser.add_argument('--patience', type=int, default=3, help='早停耐心轮数')
