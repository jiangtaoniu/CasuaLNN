...
def main():
    ...
    parser.add_argument('--model_id', type=str, default='ECL_96_96', # <-- [修改]
                        help='实验ID标识符, e.g., an identifier for the model experiment')
    parser.add_argument('--model', type=str, default='CasuaLNN',
                        help='model name, options: [CasuaLNN, TimeMixer, iTransformer, PatchTST, TimesNet, DLinear, etc.]')
    ...
    # =====================================================================================
    parser.add_argument('--data', type=str, default='custom', help='数据集类型') # <-- [修改]
    parser.add_argument('--root_path', type=str, default='./dataset/electricity/', help='数据文件根目录') # <-- [修改]
    parser.add_argument('--data_path', type=str, default='electricity.csv', help='数据文件名') # <-- [修改]
    ...
    parser.add_argument('--batch_size', type=int, default=32, help='训练批次大小') # <-- [修改]
    ...
    # =====================================================================================
    parser.add_argument('--d_model', type=int, default=16, help='模型隐藏维度') # <-- [修改]
    parser.add_argument('--d_ff', type=int, default=32, help='前馈网络中间层维度') # <-- [修改]
    parser.add_argument('--enc_in', type=int, default=321, help='编码器输入尺寸 (特征/变量数)') # <-- [修改]
    parser.add_argument('--dec_in', type=int, default=321, help='解码器输入尺寸 (恢复此参数以兼容旧模型)') # <-- [修改]
    parser.add_argument('--c_out', type=int, default=321, help='输出尺寸') # <-- [修改]
    parser.add_argument('--n_heads', type=int, default=8, help='多头注意力机制的头数')
    parser.add_argument('--e_layers', type=int, default=3, help='编码器层数 (通用)') # <-- [修改]
    ...
    parser.add_argument('--train_epochs', type=int, default=20, help='训练轮数') # <-- [修改]
